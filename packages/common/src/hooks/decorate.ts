...
export const Decorate = createHook((options: DecorateHookOptions) => {
  ...
  return (session, next) => {
    ...
    return wait(
      next(session),
      decorated => {
        const sessionInstance = session.ctx.instance;
        ...
        if (delegationKey) { // classType based decorator
          if (forkedSession.meta['adi:delegations']) {
            forkedSession.meta['adi:delegations'][delegationKey] = decorated;
          } else {
            forkedSession.meta['adi:delegations'] = {
