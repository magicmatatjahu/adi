...
import { destroyAll } from "./destroy-manager";
...
export function createProviderFactory<T>(
  ...
  let factory = (injector: Injector, session: Session) => {
    ...
    if (session.status & SessionStatus.ASYNC) {
      return createProviderAsync(provider, injections, injector, session);
    }
    const instance = new provider(...injectDeps(deps.parameters, injector, session, false));
    injectProperties(instance, deps.properties, injector, session, false);
    injectMethods(provider, instance, deps.methods, injector, session);
    return instance;
  }
...
async function createProviderAsync<T>(
  provider: Type<T>,
  injections: InjectionArguments,
  injector: Injector,
  session: Session,
) {
  const instance = new provider(...await injectDeps(injections.parameters, injector, session, true));
  await injectProperties(instance, injections.properties, injector, session, true);
  injectMethods(provider, instance, injections.methods, injector, session);
  return instance;
}
...
export function createFactory<T>(
  ...
  let factory = (injector: Injector, session: Session) => {
    ...
    return session.status & SessionStatus.ASYNC ? injectDeps(deps, injector, session, true).then(args => fn(...args)) : fn(...injectDeps(deps, injector, session, false));
  }
  if (imports || providers) {
    factory = createInjectorFactory(factory, imports, providers);
  }
...
export function createFunction<T>(
  ...
  if (imports || providers) {
    injectedFn = createInjectorFactory(injectedFn, imports, providers);
  }
  return injectedFn;
}

export function createInjectorFactory<T>(
...
function inject<T>(injector: Injector, token: Token, wrapper: Wrapper | Array<Wrapper>, metadata: InjectionMetadata, parentSession?: Session): T | undefined | Promise<T | undefined> {
  return injector.resolveToken(wrapper, Session.create(token, metadata, parentSession));
}

function injectDeps(deps: Array<InjectionArgument>, injector: Injector, session: Session, async?: false): Array<any>;
function injectDeps(deps: Array<InjectionArgument>, injector: Injector, session: Session, async?: true): Promise<Array<any>>;
function injectDeps(deps: Array<InjectionArgument>, injector: Injector, session: Session, async: boolean = false): Array<any> | Promise<Array<any>> {
  const args: Array<any> = [];
  for (let i = 0, l = deps.length; i < l; i++) {
    const arg = deps[i];
    args.push(inject(injector, arg.token, arg.wrapper, arg.metadata, session));
  };
  return async ? Promise.all(args) : args;
}

function injectProperty<T>(instance: T, prop: string | symbol, arg: InjectionArgument, injector: Injector, session: Session): any {
  return thenable(
    () => inject(injector, arg.token, arg.wrapper, arg.metadata, session),
    value => instance[prop] = value,
  );
}

function injectProperties<T>(instance: T, props: Record<string, InjectionArgument>, injector: Injector, session: Session, async?: false): void;
function injectProperties<T>(instance: T, props: Record<string, InjectionArgument>, injector: Injector, session: Session, async?: true): Promise<void>;
function injectProperties<T>(instance: T, props: Record<string, InjectionArgument>, injector: Injector, session: Session, async: boolean = false): void | Promise<void> {
  const args: Array<any> = [];
  for (const propName in props) {
    args.push(injectProperty(instance, propName, props[propName], injector, session));
  }
  // inject to symbols
  for (const sb of Object.getOwnPropertySymbols(props)) {
    args.push(injectProperty(instance, sb, props[sb as any as string], injector, session));
  }
  return (async ? Promise.all(args) : args) as unknown as void;
}

function injectMethods<T>(provider: Type<T>, instance: T, methods: Record<string, InjectionMethod>, injector: Injector, parentSession: Session): void {
...
function injectFunction<T>(
  ...
  return function func(injector: Injector, session: Session, ...args: any[]) {
    ...
    return thenable(
      ...
      value => {
        if (toRemove) destroyAll(toRemove);
