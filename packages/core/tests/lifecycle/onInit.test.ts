import { Injector, Injectable, OnInit, OnInitLifecycle, Inject, TransientScope } from "../../src";
...
describe('onInit', function() {
    ...
    const injector = Injector.create([
      {
        ...
        hooks: [
          OnInitLifecycle(hook3), 
          OnInitLifecycle(hook2), 
          OnInitLifecycle(hook1),
        ],
      }
    ]).init() as Injector;
  ...
  test('should work in injection based hook', function() {
    ...
    class Service {
      constructor(
        @Inject([
          OnInitLifecycle(hook('injection onInit'))
        ])
        readonly testService1: TestService,
      ) {}
    }
  ...
  test('should work alongside with all injection possibilities', function() {
    ...
    @Injectable({
      hooks: [
        OnInitLifecycle(hook('definition onInit')),
      ]
    })
    class TestService implements OnInit {
      onInit() {
    ...
    const injector = Injector.create([
      ...
      TestService,
      {
        provide: TestService,
        hooks: [
          OnInitLifecycle(hook('provider onInit')),
        ],
      }
    ]).init() as Injector;
